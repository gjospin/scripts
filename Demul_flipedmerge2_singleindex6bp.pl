...
while (<INBC>) {
	...
	unless (exists $mapping_file{ $line[0] }{"BarcodeSequence"}){
	    print STDERR "**** WARNING **** Did not find BarcodeSequence values.\nSkipping $_\n";
	    next;
	}
	$samples{ $line[0] } = 1;
	...
	my $code_original = $mapping_file{ $line[0] }{"BarcodeSequence"};
...
#		    $index2[$i] = <INDEX2>;
	    ...
	    #
	    ## FOR THIS RUN ONLY the index read was read as 8BP
	    ## but our real barcodes are 6bp so need to substr
	    #
	    my $i1 = substr($index1[1],0,6);
#	    my $i2 = $index2[1];
	    chomp($i1);
#	    chomp($i2);
	    $total_codes{$i1} = 0 unless exists $total_codes{$i1};
	    $total_codes{$i1}++;
	    my $long_code = $i1;
...
##close files and flush IO buffers
	...
	$source_dir= "qiime_ready";
        if($trim_path && $adaptor_file){
            ...
            my $trim_cmd = "java -jar $trim_path/trimmomatic.jar SE -phred33 \"$output_dir/qiime_ready/$out_file_core".".$handle".".extendedFrags.fastq\" \"$output_dir/trimmed_fastq/$out_file_core".".$handle.trim\" ILLUMINACLIP:$adaptor_file:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:50 >/dev/null 2>/dev/null";
