...
# Date : Jan 13th 2014
#
# Usage : >perl ab1_to_merged_fasta.pl <forward>.ab1 <reverse>.ab1
#
...
my $aln_obj = Bio::SeqIO->new(-file=>"temp_aln.aln",-format=>'fasta');
#`rm test_aln.aln test_aln.fa`;
my $seq1 = $aln_obj->next_seq;
my $seq2 = $aln_obj->next_seq;
my $merged_seq = merge_alignment(read1=>$seq1->seq, read2=>$seq2->seq, quality1 => $read1[3], quality2=> $read2[3]);
...
# write out read length after Qtrim.

=head2 ab1_to_fastq

reads 1 abi file and outputs a fastq file
<filename>.ab1 to <filename>.fastq

returns the name of the fastq file

=cut
sub ab1_to_fastq{
    my %args = @_;
    my $file = $args{file};
    my $seqio_obj = Bio::SeqIO->new(-file=>$file, -format=>"abi");
    $file =~ /([^\/]*).ab1$/;
    my $id = $1;
    my $seq_obj = $seqio_obj->next_seq();
    my $out = Bio::SeqIO->new(-file=>">$id.fq" , -format=>"fastq");
    $out->write_seq($seq_obj);
    return "$id.fq";
}
#system("ls -lh *.fq > /dev/null");
